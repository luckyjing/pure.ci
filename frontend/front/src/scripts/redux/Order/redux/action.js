...
export function getInfo(text) {
  return (dispatch, getState) => {
    ...
    return $.ajax({
      url:"/administrator/ajax/searchOrder.json",
      data:{
        text
      },
      dataType:'json'
...
export function getInfo2(orderNumber) {
  return (dispatch,getState)=>{
    return $.ajax({
      url:"/administrator/ajax/orderPay.json",
      data:{
        orderNumber
      },
      dataType:'json'
        ...
        if (res.code==200){
          alert("The Product is succeed to pay!");
...
export function getOrderList() {
  return (dispatch, getState) => {
    // 在这儿使用dispath可以实现在异步请求前做一次操作
    return $.ajax({
      url:"/administrator/ajax/orderList.json",
      data:{
