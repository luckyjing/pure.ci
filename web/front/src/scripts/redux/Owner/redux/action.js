...
export function getInfo() {
  return (dispatch, getState) => {
    ...
    dispatch(loading());
    return $.ajax({
      url:"/administrator/ajax/ownerList.json",
      data:{
      },
      dataType:'json'
    }).done(res=>{
...
export function addBLCus(index) {
  return {
    type:TYPE.ADBLCUS,
    index
  }
...
export function addBlackCus(userId,index) {
  return (dispatch, getState) => {
    // 在这儿使用dispath可以实现在异步请求前做一次操作
    return $.ajax({
      ...
      if (res.code==200){
        dispatch(addBLCus(index))
      }else{
        alert("The merchant has orders not processed, temporarily can not be pulled black!");
      }
    });
  }
}

export function deleteCust(index) {
  return{
    type:TYPE.DELETECUS,
    index
  }
...
export function deleteCus(userId,index) {
  return (dispatch, getState) => {
    // 在这儿使用dispath可以实现在异步请求前做一次操作
    return $.ajax({
      url:"/administrator/ajax/user_deleteUser.json",
      data:{
        userId
      },
      dataType:'json'
